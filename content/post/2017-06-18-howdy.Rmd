...
tags:
  ...
  - Shiny
  - Plot
description: 'A short introductory post accompanying the R code for generating the website logo.'
output:
  blogdown::html_page:
    fig_retina: 2
    fig_width: 7
...
This webpage is built by using the `blogdown` package in R. The code itself is powered by [Hugo](https://themes.gohugo.io/), and I use a variation of the theme [cactus plus](https://themes.gohugo.io/hugo-theme-cactus-plus/).
As per the recommendation of Yihui Xie[^2], I host my site on [netlify.com](https://netlify.com), which offers continuous deployment from a git repository. All I have to do is to create a new Markdown/RMarkdown document and within seconds it will be deployed to the website. Pretty neat.
...
patch <- read_csv("patch.csv") #Sequence data for included publications
...
htmlwidgets::onRender(
      sunburst(patch, count = TRUE,
               ...
               colors = c("", blues[1:8], reds[7:2]), #For some reason, had to assign a blank colour first for
...
[^2]: [General recommendation](https://bookdown.org/yihui/blogdown/workflow.html), not through personal correspondence.