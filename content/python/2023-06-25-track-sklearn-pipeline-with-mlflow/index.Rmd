...
tags:
  - sklearn
  - mlflow
description: "Logging sklearn gridsearch CV data to MLflow and deploy the best model to make inferences"
---
...
I am a simple person. I just want to neatly log all the parameters and metrics pertaining to a `sk-learn` pipeline to MLflow, including cross-validation. But I don't want to fit and log one model at a time, and the MLflow autologger doesn't save the results of intermediate stages when tracking meta-estimators (i.e. sklearn pipelines). I browsed 20+ tutorials, blog posts, and what-have-you. None of them does it the way I need it.
...
Next, a pipeline object to prevent data leakage and make our lives easier
...
Now, I want to do a hyper-parameter grid search using cross-validation. It may not be crucial with the synthetic dataset, but sometimes I'm not sure which scaler is a better choice given the data, so I include both `MinMaxScaler()` and `StandardScaler()`, as well as the option to skipp the step altogether with `passthrough`. I also supply some options for the logistic regression as well.
...
Making coffee while it trains ☕ This fits about 180 models.
...
I only want columns that either contain the substring `mean` (all metrics) or `param_` (hyper-parameters). Also sorting the `df` column names alphabetically to make it easier downstream---i.e. when viewing them later on the MLflow UI.
...
This on is a bit messy, but essentially:
...
4. We use the context manager `with mlflow.start_run():` to start logging a series of runs. Normally, you would fit the model inside, and MLflow would capture it. Here, I only want to upload the CV results and deploy the best model online, so I fit the model outside.

5. We save the `run_id` because we will need it to make the call for model serving.

6. In the meanwhile, we log the bits we are interested in, including the data with `mlflow.log_input()`.

7. We log the model using `mlflow.sklearn.log_model()`---most popular frameworks exist as flavours in MLflow. We supply the model signature, which is inferred from the training and predicted data (i.e. acceptable shape of input and output data). We also register the model by giving it a name so that we can refer to it easily during staging.

8. Next, we loop the child runs under the parent. These have their own context manager. With `nested=True`, they will be grouped under the parent run in the MLflow UI for easier viewing.
